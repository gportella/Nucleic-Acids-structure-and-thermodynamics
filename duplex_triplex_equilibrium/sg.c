// From the web, does Savitzky-Golay smoothing. Seems adapted
...
void sgsmooth(double *x[],int nts, double *smth[])  {
  ...
  for (i=1; i<nts; i++) smoothed[i]=x[1][i];  //save unsmoothed smoothed
  for (i=1; i<nts; i++) smth[0][i]=x[0][i];  //save unsmoothed smoothed
  for (i=1; i<nts; i++) ysave[i]=x[1][i];  //save unsmoothed smoothed

  nl=12; nr=12; m=6;                            //see savgol
  ...
  for (i=1; i<=ndata-nr; i++) {
    ...
    for (j=1; j<=nl+nr+1; j++){
	  if (i+ind[j]>0){   //skip left points that do not exist.
        smoothed[i] += c[j]*ysave[i+ind[j]];
        smth[1][i] += c[j]*ysave[i+ind[j]];
	  }
	}
  }
