// GPC 10/03/2015 g_qrdc_ensemble v 0.1 
//
...
//
// Ensemble averaging not implemented
//
...
real compute_av_qfact_e( rdc_str_e *rdc, int n_rdc, int nframes )
{
//
...
real compute_rdc_qfact_e(rdc_str_e *rdc, int n_rep, int n_rdc, int natoms, rvec **x, bool bCorrel)
{
//
...
// bCorrel == True, and with the actual scaling factor otherwise.  Actually we
// don't really care very much about the per frame Q-factor, it's best to
// compute the ensemble averaged value.
...
//
    ...
    if (bCorrel){
        ...
        for (i=0; i< n_rdc ; i++){
            // if we are not touching the correlation
            // we will add up the computed RDCs 
            // to get the ensemble value later on
...
void find_best_orient_e(rdc_str_e *rdc, int n_rep, int n_rdc, int natoms, rvec *x, int max_iter, 
...
// Q-factor. The rdc_str structure containts then the best rdcs per frame, and
...
//
	...
	// initiate qfact
	corr_b4 = compute_rdc_qfact_e(rdc, n_rep, n_rdc, natoms, xens, bCorrel);
	max_corr = corr_b4;
	int acc=0;
	do {
		count++;
		...
		corr = compute_rdc_qfact_e(rdc, n_rep, n_rdc, natoms, xnew_e, bCorrel);
		ran_met = ((real) rand() / (real) RAND_MAX )*(1);
		// Small changes (compared to inv_T) are not as easily accepted as large ones
		// I still don't get why it should not get trapped
...
//	if (exp((corr_b4-corr)/inv_T) < ran_met  ){
			if (corr > max_corr){
				...
				// should I copy everything? in principle only r_rot should have changed
				for(r=0;r<n_rep;r++){
					for(i=0;i<natoms; i++){
		...
		//printf("iter %d acc %d ratio %f\n",count, acc, (real)acc/(real)count);
	} while ( max_corr < 0.999  && count < max_iter );

	// rdc structure has been updated with newest computed values
	corr=compute_rdc_qfact_e(rdc, n_rep, n_rdc, natoms, xopt_e, bCorrel);
	if (bVerbose) {
	...
	bCorrel=FALSE;
	qfact=compute_rdc_qfact_e(rdc, n_rep, n_rdc, natoms, xopt_e, bCorrel);
	if (bVerbose){
		printf("Per frame Q-fact is %f \n",qfact);
	}
	// Copy new orientation for output gmx traj
	// In the future this could be an option and not the default
	//for(i=0;i<natoms; i++){copy_rvec(xopt[i],x[i]);}
	sfree(xnew);
	for(r=0; r< n_rep; r++){
		sfree(xnew_e[r]);
		sfree(xopt_e[r]);
...
int main(int argc, char *argv[])
{
	...
	// Write xtc, could be made optional
    ...
    if((rdc_e = (rdc_str_e *) calloc(n_rdc, sizeof(rdc_str_e))) == NULL) {
		printf("\nERROR, cannot allocate rdc_str\n"); exit(1);
	}
	for (i=0; i<n_rdc; i++) {
		snew(rdc_e[i].comp,n_rep);
	}
	// Read in the RDCs 
	// Crudest parsing, no comments or extra lines allowed
    j=0;
    while(fgets(line, 255, f_rdc) != NULL)
	{
		sscanf(line, "%d %d %f %f", &rdc_e[j].ind[0], &rdc_e[j].ind[1], &rdc_e[j].exp, &rdc_e[j].gyr);
		j++;
	}
	...
	do {
		...
		find_best_orient_e(rdc_e, n_rep, n_rdc, natoms, x, max_iter, mc_half_step, inv_T, bVerbose ) ; 
	...
	av_qfact=compute_av_qfact_e( rdc_e, n_rdc, nframes ); 
