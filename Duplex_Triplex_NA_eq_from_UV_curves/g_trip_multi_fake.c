...
void do_solve_eq(double *x, int n, f_info p, double *fd, double *ft);
	
void create_fit_curve(double *x[],double *xf[],int n,f_info s)
{
	int i; 
...
f_info do_fit_triplex(double *xx[],int n,f_info p, bool bVerbose)
{
	int i; 
	int iter=0;
	int status;
...
double suggest_tm3(double *ctrip[],int nts, double dT)
{
	...
	tm3=0;
	for(i=PAD; i< nts-PAD-1; i++){ 
		if (smth[0][i] < dT-10 && bMax[i] && smth[1][i] > val) {
			val=smth[1][i];
			tm3=smth[0][i];
		}
	}
	if(tm3 == 0 || tm3 > dT-5) {tm3=50+273.15;}
...
#define NFILE asize(fnm)
	...
	for(i=0; i<nfiles; i++){
		...
		p.dh2= f_inp[i].dh2 ;
		p.tm2= f_inp[i].tm2 + 273.15 ;

		max_abs=find_maxabs(f_inp[i].x,f_inp[i].nts,tmaxabs);
		t_height=find_t_height(f_inp[i].x,f_inp[i].nts,max_abs);
		...
		scale_data(f_inp[i].x,f_inp[i].nts,max_abs);
		p.tm3=suggest_tm3(f_inp[i].x,f_inp[i].nts, p.tm2);
		p.dh3=-80;
		p.c = c_fix;
		if(bVerbose){
		...
		//do the fit
		solution=do_fit_triplex(f_inp[i].x,f_inp[i].nts,p,bVerbose);
		snew(ftrip[0],f_inp[i].nts);
		snew(ftrip[1],f_inp[i].nts);
